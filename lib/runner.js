...
function runner(script, payload, options, callback) {
  ...
  const auth = script.config.auth;
  if () {
    if (_.isArray(auth[0])) {
      script.config.auth = [
        {
          fields: script.config.auth.fields,
          reader: createReader(script.config.auth.order),
          data: auth
        }
      ];
    } else {
      _.each(script.config.auth, function(el) {
        el.reader = createReader(el.order);
      });
    }
  } else {
    script.config.auth = null;
  }
...
function createContext(script) {
  ...
  result.tokenTarget = script.config.targetToken;
  result._uid = uuid.v4();
  return result;
}
